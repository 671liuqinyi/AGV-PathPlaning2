...
from algorithm.DQN_PURE_structure.DQN_PURE import Agent
...
DQN_MODEL_DIR = "algorithm/DQN_PURE_structure/923631"  # e.g. "algorithm/DQN_PURE_structure/923631"
...
def resolve_dqn_model_path_candidates(base_dir=None):
    policy_candidates = ["RMFS_DQN_PURE_policy_net.pt", "RMFS_DQN_PURE_policy_net_auto.pt"]
    target_candidates = ["RMFS_DQN_PURE_target_net.pt", "RMFS_DQN_PURE_target_net_auto.pt"]

    dqn_root = os.path.join(SRC_DIR, "algorithm", "DQN_PURE_structure")

    if base_dir is None:
        preferred = os.path.join(dqn_root, "923631")
        if os.path.isdir(preferred):
            ...
            if not dirs:
                raise FileNotFoundError("No pure DQN model folder found. Train with src/train/dqn_pure.py first.")
    ...
    if not candidates:
        raise FileNotFoundError(f"No usable pure DQN model files found under: {base_dir}")
...
def load_dqn_agent(model_dir=None, layout_h=None, layout_w=None):
    ...
    for base in base_dirs:
        try:
            ...
            continue
    if not candidates:
        raise FileNotFoundError("No usable pure DQN model files found in configured directories.")
    ...
    if fallback is None:
        raise RuntimeError("pure DQN model loading failed for all candidates:\n" + "\n".join(checked))

    if pooled_current is not None:
        debug = "\n".join(checked)
        raise RuntimeError(
            "pure DQN model input size mismatch (no-resize mode). "
