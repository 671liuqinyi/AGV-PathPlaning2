...
# Read test data from data.json with these keys
# Set to None to use keys from data.json metadata:
# _latest_layout_key / _latest_tasks_key
# LAYOUT_KEY = "layout_list_15"
# TASKS_KEY = "tasks_15_generated"
...
def resolve_model_paths(base_dir=None):
    ...
    if base_dir is None:
        ...
        if os.path.isdir(preferred):
            ...
            dirs = [
                ...
                or os.path.isdir(os.path.join(d, "test"))
            ]
            if not dirs:
                raise FileNotFoundError(
                    "No MAPPO model folder found. Train with main4.py first."
                )
    ...
    raise FileNotFoundError(f"No usable MAPPO model files found under: {base_dir}")
...
def load_layout_and_tasks_from_json(layout_key, tasks_key):
    ...
    if layout_key is None or tasks_key is None:
        raise KeyError(
            "Missing layout/tasks key and no _latest_* metadata in data.json"
        )
...
def create_action_mask(valid_path_matrix, current_place):
    ...
    for action, (dx, dy) in ACTION_TO_OFFSET.items():
        ...
        if (
            1 <= nx <= map_w
            and 1 <= ny <= map_h
            and valid_path_matrix[ny - 1][nx - 1] != 0
        ):
...
def load_agent(model_dir=None):
    ...
    actor_net.eval()
    critic_net.eval()
    return MAPPOAgent(actor_net, critic_net, device), actor_path, critic_path
...
def validate_tasks(task_list):
    ...
    for idx, task in enumerate(task_list, start=1):
        if not isinstance(task, dict) or len(task) != 1:
            raise ValueError(
                f"task[{idx}] must be a dict with exactly one vehicle key."
            )
...
def run_mappo_test(layout_matrix, task_list, max_steps_per_task=120, model_dir=None):
    ...
    for idx, task in enumerate(task_list, start=1):
        ...
        for _ in range(max_steps_per_task):
            ...
            obs, cp, _, valid_path_matrix = state_manager.create_state(
                all_info, veh_name, obs_clip=True
            )
            ...
            if is_valid_next(valid_path_matrix, next_pos):
                task_distance += float(
                    np.linalg.norm(np.array(next_pos) - np.array(current))
                )
                current = next_pos
            else:
                task_collisions += 1
