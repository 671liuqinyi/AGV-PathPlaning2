...
MODEL_DIR = "algorithm/MAPPO_MOE_structure/823632/train/run1"
...
def resolve_model_paths(base_dir=None):
    actor_candidates = ["RMFS_MAPPO_MOE_actor_net.pt", "RMFS_MAPPO_MOE_actor_net_auto.pt"]
    critic_candidates = ["RMFS_MAPPO_MOE_critic_net.pt", "RMFS_MAPPO_MOE_critic_net_auto.pt"]

    mappo_root = os.path.join(SRC_DIR, "algorithm", "MAPPO_MOE_structure")
    ...
    if base_dir is None:
        ...
        if os.path.isdir(preferred):
            ...
            if not dirs:
                raise FileNotFoundError("No MAPPO+MoE model folder found. Train with mappo_moe.py first.")
    ...
    # Case 1: base_dir itself already contains actor/critic files
    # (e.g. directly point to a specific run folder).
    ...
    raise FileNotFoundError(f"No usable MAPPO+MoE model files found under: {base_dir}")
...
def load_agent(model_dir=None):
    import torch
    from algorithm.MAPPO_MOE_structure.MAPPO_MOE import MAPPOMoEAgent
    ...
    actor_net.eval()
    critic_net.eval()
    return MAPPOMoEAgent(actor_net, critic_net, device), actor_path, critic_path
...
def run_mappo_moe_test(layout_matrix, task_list, max_steps_per_task=120, model_dir=None):
