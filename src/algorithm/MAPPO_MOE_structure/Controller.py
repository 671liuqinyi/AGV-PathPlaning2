...
from algorithm.MAPPO_MOE_structure.MAPPO_MOE import CriticNet, MAPPOMoEAgent, MoEActorNet
from algorithm.MAPPO_MOE_structure.features import extract_expert_features
...
class MAPPOMoEAgentController:
    """Bridge between Scene and MAPPO+MoE algorithm."""
    ...
    def __init__(
        ...
        self.base_save_path = os.path.join(SRC_DIR, "algorithm", "MAPPO_MOE_structure", "823632")
        ...
        print("start simulation with MAPPO+MoE algorithm")
        ...
        self.expert_gate_sum = np.zeros(3, dtype=np.float64)
        self.expert_gate_count = 0

    def create_agent(self):
        if self.control_mode == "train_NN":
            print("create MAPPO+MoE NN")
            actor_net = MoEActorNet(self.state_number, self.rmfs_model.action_number)
            critic_net = CriticNet(self.global_state_dim)
        else:
            print("load MAPPO+MoE NN")
            actor_path = self._resolve_model_path("RMFS_MAPPO_MOE_actor_net.pt")
            critic_path = self._resolve_model_path("RMFS_MAPPO_MOE_critic_net.pt")
            actor_net = torch.load(actor_path, map_location=self.device)
            critic_net = torch.load(critic_path, map_location=self.device)

        self.agent = MAPPOMoEAgent(actor_net, critic_net, self.device)

    def self_init(self):
        self.reward_acc = 0
        self.veh_group = []
        self.action_length_record = 0
        self.expert_gate_sum[:] = 0.0
        self.expert_gate_count = 0
    ...
    def model_run(self, run_mode):
        ...
        for i_episode in range(self.simulation_times):
            ...
            gate_mean = self.expert_gate_sum / max(self.expert_gate_count, 1)
            log = (
                "i_episode {}\treward_accu: {} \taction_length: {} \tepisode_cost: {:.2f}s "
                "\tgate=[{:.3f},{:.3f},{:.3f}]".format(
                    i_episode,
                    self.reward_acc,
        ...
        if run_mode == "train_NN":
            ...
            saveManager.draw_picture(
                self.agent.moe_balance_value,
                title="MoE Balance Loss",
                x_label="training episodes",
                y_label="loss",
                color="m",
                save_path=os.path.join(self.current_save_dir, "MoE Balance Loss"),
            )
            plt.show()
    ...
    def choose_action(self, all_info, this_veh):
        ...
        action, log_prob, gate_weights = self.agent.choose_action(
            obs,
        ...
        self.expert_gate_sum += gate_weights
        self.expert_gate_count += 1
    ...
    def save_neural_network(self, auto=False):
        ...
        if auto:
            torch.save(self.agent.actor_net, os.path.join(self.current_save_dir, "RMFS_MAPPO_MOE_actor_net_auto.pt"))
            torch.save(self.agent.critic_net, os.path.join(self.current_save_dir, "RMFS_MAPPO_MOE_critic_net_auto.pt"))
        else:
            torch.save(self.agent.actor_net, os.path.join(self.current_save_dir, "RMFS_MAPPO_MOE_actor_net.pt"))
            torch.save(self.agent.critic_net, os.path.join(self.current_save_dir, "RMFS_MAPPO_MOE_critic_net.pt"))
    ...
    def _resolve_model_path(self, model_name):
        ...
        if not os.path.isdir(train_dir):
            raise FileNotFoundError("No trained MAPPO+MoE model found in test or train folders.")
